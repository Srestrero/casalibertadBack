...
public class ProgramasCasaLibService {
    ...
    public ProgramasCasaLib crearProgramasCasaLib(String numeroDocumento,
            ...
            if(programas==null){
            //programas =  programasCasaLibRepository.findByUsuarios(usuarios);
            ProgramasCasaLib nuevoPrograma= new ProgramasCasaLib();
            //programas = new ProgramasCasaLib();
            nuevoPrograma.setOtroMedioConoc(consultaProgramasCasaLibDTO.getOtr_medio_conoc());
            nuevoPrograma.setNombresOtroFunc(consultaProgramasCasaLibDTO.getNombres_otro_func());
            MediosConocimiento medios = mediosConocimientoRepository.findByUniqid(consultaProgramasCasaLibDTO.getMedios_conoc_uniqid());
            nuevoPrograma.setMediosConocs(medios);
            Expectativas expectativas = expectativasRepository.findByUniqid(consultaProgramasCasaLibDTO.getExpectativas_uniqid());
            nuevoPrograma.setExpectativas(expectativas);
            ...
            nuevoPrograma.setFuncionarioRegistros(funcionario);
            nuevoPrograma.setUsuarios(usuarios);

            return programasCasaLibRepository.save(nuevoPrograma);
            }else{
                try {
                    actualizaProgramasCasaLib(numeroDocumento,consultaProgramasCasaLibDTO);
                } catch (Exception ex) {
                    Logger.getLogger(ProgramasCasaLibService.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            return programasCasaLibRepository.findByUsuarios(usuarios);
